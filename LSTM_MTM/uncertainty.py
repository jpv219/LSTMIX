...
#         pertb = np.random.normal(loc=0.0, scale=pertb_scale, size=input_seq.shape)
...
def plot_ensem_pred(model_name,features,scale,c_idx,
    ...
    for f_idx in range(num_features):
        ...
        fig.savefig(os.path.join(fig_savepath, f'Perturbed_rollout_{model_name}_{features[f_idx]}.png'), dpi=150)
        plt.show()
