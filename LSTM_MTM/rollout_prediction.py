...
class Rollout(PathConfig):
    ...
    @profile(precision=4)
    def rollout(self,model, input_seq, total_steps):
        ...
        with torch.no_grad():
            ...
            num_forwards = int(total_steps / self.steps_out) + 1
    ...
    def plot_model_pred(self,model,fine_labels,model_name,features,set_labels,set,
                        X_data,true_data,casebatch_len,inset_on=False):
        ...
        for f_idx in range(0,2):
            ...
            x_range = range(self.wind_size-self.steps_out,len(true_data)- self.steps_out+1)
            s_idx_l = -1
            x_range_l = range(self.wind_size,len(true_data)+1)
            ...
            for seq, case in enumerate(set_labels):
                ...
                if seq == 0:
                    ...
                    if axins is not None:
                        ...
                        axins.set_xlim(self.wind_size-1, len(true_data)+5)
                else:
                    ax.plot(x_range_l,
    ...
    def plot_y_x(self,model, model_name,set_labels, features,
        ...
        for seq, case in enumerate(set_labels):
            ...
            for f_idx in range(num_features):
                ...
                for win_tidx in range(0, self.steps_out):

                    if not label_added:
                        if seq ==0:
                            # True data idxs to match nth window element sequence for all windows (size of true data equals number of windows)
                            start_idx = self.steps_in + win_tidx # start idx in true data extends for the number of steps out (elements in output window)
        ...
        plt.savefig(os.path.join(self.fig_savepath,'windowed',f'{model_name}', f'y_x_{type}_{model_name}.png'), dpi=200)
        plt.show()

    # y_x error dispersion for rollout predictions
    def plot_rollout_yx(self,rollout_seq, true_data, set_labels, features, model_name,dpi=200, train_val = None):
        ...
        for seq, case in enumerate(set_labels):
            ...
            for f_idx in range(num_features):
                ...
                plt.plot(true_data[self.steps_in:,seq,f_idx],rollout_seq[seq, self.steps_in:len(true_data),f_idx], 
                    marker = 'o', linestyle = 'None', markersize = 2, color = colors[seq],
                    label=plot_label if f_idx == 0 else "")
                
                ground_truth.append(true_data[self.steps_in:,seq,f_idx])
                predictions.append(rollout_seq[seq, self.steps_in:len(true_data),f_idx])
        ...
        if train_val == 'train' or train_val == 'val':
            plt.savefig(os.path.join(self.fig_savepath,'rollouts',f'{model_name}', f'y_x_roll_{train_val}_{model_name}.png'), dpi=200)
        plt.savefig(os.path.join(self.fig_savepath,'rollouts',f'{model_name}', f'y_x_roll_{model_name}.png'), dpi=200)
        plt.show()   
...
def main():
    ...
    if pred_choice.lower() == 'y' or pred_choice.lower() == 'yes':
        ...
        rollout.plot_model_pred(model, fine_labels, model_choice, features, splitset_labels[0],
                        'Train',X_train, train_arr,train_casebatch)
        
        rollout.plot_model_pred(model, fine_labels, model_choice, features, splitset_labels[1],
                'Validation',X_val, val_arr, val_casebatch)
    ...
    input_seq = test_arr[:steps_in,:,:]

    # Total steps to predict
    total_steps = test_arr.shape[0] - steps_in

    ## Calling rollout prediction for test data
    logtest_path = os.path.join(path.fig_savepath,'performance_logs',f'{model}_testroll.txt')

    with open(logtest_path,'w') as logfile:
        sys.stdout = logfile
        rollout_seq = rollout.rollout(model,input_seq,total_steps)

    ## Calling rollout on training and validation data

    input_train = train_arr[:steps_in,:,:]
    train_steps = train_arr.shape[0] - steps_in

    input_val = val_arr[:steps_in,:,:]
    val_steps = val_arr.shape[0] - steps_in
    
    # Saving performance in a log
    logtrain_path = os.path.join(path.fig_savepath,'performance_logs',f'{model}_trainroll.txt')

    with open(logtrain_path,'w') as logfile:
        sys.stdout = logfile
        rollout_train = rollout.rollout(model,input_train,train_steps)
        rollout_val = rollout.rollout(model,input_val,val_steps)
    ...
    if xy_trainval_choice.lower() == 'y' or xy_trainval_choice.lower() == 'yes':
        rollout.plot_rollout_yx(rollout_train,train_arr,splitset_labels[0],features,model_choice,train_val='train')

        rollout.plot_rollout_yx(rollout_val,val_arr,splitset_labels[1],features,model_choice,train_val='val')
    ...
    if test_arr.shape[-1] == 2:
        ...
        rollout.plot_rollout_yx(rollout_seq,test_arr,splitset_labels[2],features, model_choice)
    
        rollout.plot_rollout_pred(rollout_seq,test_arr,features,splitset_labels[2], model_choice)
