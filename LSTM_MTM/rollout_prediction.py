...
# trainedmod_savepath = '/home/fl18/Desktop/automatework/ML_casestudy/LSTM_SMX/LSTM_MTM/trained_svALLmodels/'
trainedmod_savepath = '/home/fl18/Downloads/'
...
def plot_all_rollout_pred(rollout_seq, true_data, input_steps, features,set_labels, model_name):
    ...
    fig.supxlabel('Time steps', fontsize=30)
    fig.supylabel('Estimated density of Drop count in drop size ranges', fontsize=30)
    fig.suptitle(f'Rollout pred with LSTM {model_name} for Drop count in drop size range', fontsize=40)
    fig.savefig(os.path.join(fig_savepath, f'Rollout_{model_name}_DSD.png'), dpi=150)
    plt.show()
